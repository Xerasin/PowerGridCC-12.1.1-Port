...
public class VariacPeripheral extends AbstractPowerGridPeripheral<VariacBlockEntity> {
   public VariacPeripheral(VariacBlockEntity variac) {
      super(variac);
   }

   public String getType() {
      return "powergrid_variac";
   }

   public Set<String> getAdditionalTypes() {
      return Set.of("powergrid_electric", "powergrid_tuned", "powergrid_transformer");
   }

   private ThermalBehaviour getThermalBehaviour() {
      return (ThermalBehaviour)((VariacBlockEntity)this.target).getBehaviour(ThermalBehaviour.TYPE);
   }
   ...
   public final double getRatio() {
      return (double)((VariacBlockEntity)this.target).getRatio();
   }
   ...
   public final void setRatio(double ratio) throws LuaException {
      if (!Double.isFinite(ratio)) {
      ...
      } else if (!(ratio < 0.01) && !(ratio > (double)1.0F)) {
         ...
         throw new LuaException("Variac ratio must be between 0.01 and 1.0");
      }
   }
